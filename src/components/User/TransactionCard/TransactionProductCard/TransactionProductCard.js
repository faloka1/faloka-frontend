    ...
    return (
        <Row as={Link} className="card-product" to={isLoading ? '#' : `/products/${product.products.slug}`}>
            ...
            <Col className={`product-detail ${isLoading ? 'placeholder-glow' : ''}`}>
                ...
                <small className={`product-size ${isLoading ? 'col-1 placeholder bg-secondary' : ''}`}>{isLoading ? (null):(<span>Ukuran : <small className="size-value">{product.variants.name}</small></span>)}</small>
